...
class Calculator(Node):
    ...
    def __init__(self):
        ...
        self.arithmetic_argument_subscriber = self.create_subscription(
            ArithmeticArgument, #토픽타입
            'arithmetic_argument', #토픽이름
            self.get_arithmetic_argument, #콜백함수
            QOS_RKL10V,#QOS 설정
            callback_group=self.callback_group) #-> callback_group 설정, 이유: action server와 service server가 동시에 요청올 수 있으므로
